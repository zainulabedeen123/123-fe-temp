    ...
    },
    {
      ...
      render: (value, row, index) => (
        <div className="flex items-center gap-2 w-[42px]">
          <FaRegEdit size={18} className='cursor-pointer' onClick={() => navigate('/organization/admin/dashboard/edit')} />
          {row.status === 'Active' ? (
      ...
      <div className='bg-[#2D363D] rounded-2xl p-5 flex flex-col justify-start w-full pb-10'>
        <div className='flex items-center justify-between gap-2 mb-5 flex-wrap'>
          ...
          <div className='flex gap-2'>
            ...
            <button className='yellowBG text-white text-[12px] rounded-[4px] px-2 py-1 text-sm cursor-pointer min-w-[110px]' onClick={() => navigate('/organization/admin/dashboard/add')}>Add New User</button>
          </div>
        </div>
